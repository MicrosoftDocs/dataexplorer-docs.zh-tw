---
title: 'quantize_udf ( # A1-Azure 資料總管'
description: '本文說明 Azure 資料總管中 ( # A1 使用者定義函數的 quantize_udf。'
author: orspod
ms.author: orspodek
...
ms.locfileid: "89557826"
---
# <a name="quantize_udf"></a>quantize_udf ( # A1


Function bin 計量資料 `quantize_udf()` 行。 它會根據 K 表示演算法，將計量資料行 quantizes 至類別標籤。
...
`T | invoke quantize_udf(`*num_bins* `,`*in_cols* `,`*out_cols* `,`*標籤*`)`
...
* `quantize_udf()` 這是使用者定義函數。 您可以在查詢中內嵌其程式碼，或將其安裝在您的資料庫中：
...
* `quantize_udf()`是要使用[invoke 運算子](../query/invokeoperator.md)套用的[表格式函數](../query/functions/user-defined-functions.md#tabular-function)
