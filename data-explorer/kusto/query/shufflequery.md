...
使用查詢參數`hint.strategy = shuffle`或`hint.shufflekey = <key>`來設定隨機查詢策略。
...
當索引鍵（`join`索引鍵、索引`summarize`鍵或`make-series`索引鍵）具有高基數和一般查詢策略點擊查詢限制時，使用隨機查詢策略會很有用。
...
如果複合索引鍵不是唯一的，但每個索引鍵都不是唯一`hint`的，請使用此索引鍵，依隨機運算子的所有索引鍵隨機播放資料。
...
如果您套用`hint.strategy=shuffle` （而不是在查詢計劃期間忽略策略），並依複合索引鍵 [`ActivityId`， `numeric_column`] 隨機播放資料，則結果將會不正確。
`summarize`運算子位於`join`運算子的左邊。 這個運算子會依索引`join`鍵的子集分組，在我們的案例中為`ActivityId`。 因此， `summarize`會使用索引鍵來分組`ActivityId`，而資料則是以複合索引鍵 [`ActivityId`， `numeric_column`] 進行分割。
隨機 by 複合索引鍵 [`ActivityId`， `numeric_column`] 不一定表示索引鍵`ActivityId`的隨機是有效的，而且結果可能不正確。
...
這兩個記錄的複合索引鍵都對應到不同的`ActivityId`資料分割（56和65），但這兩個記錄的值相同。 左側`summarize`的運算子`join`需要資料行`ActivityId`的類似值，才會在同一個分割區中。 此查詢會產生不正確的結果。

若要解決這個問題，您`hint.shufflekey`可以使用來指定聯結上的隨機播放`hint.shufflekey = ActivityId`索引鍵。 這個金鑰組于所有可隨機操作的運算子而言都是常見的。
在此情況下，隨機是安全的， `join`因為`summarize`和都是以相同的金鑰隨機播放。 因此，所有類似的值都會位於相同的分割區中，且結果會正確：
...
在隨機查詢中，預設的分割區編號是叢集節點編號。 您可以使用語法`hint.num_partitions = total_partitions`來覆寫這個數位，這會控制分割區的數目。
...
執行一般`summarize`策略時，查詢會在1:08 之後結束，而記憶體使用量尖峰為 ~ 3 GB：
...
執行查詢時， `hint.num_partitions`不會使用兩個分割區（作為叢集節點編號），而下列查詢將會花費 ~ 1:10 分鐘：
...
左側資料表具有1,500 萬次記錄，其中索引`join`鍵的基數為 ~ 14M。 的右側`join`是使用150M 記錄，而索引`join`鍵的基數是1千萬個。
執行的一般策略時`join`，查詢會在 ~ 28 秒後結束，而記憶體使用量尖峰為 1.43 GB：
...
在較大的資料集上嘗試相同的`join`查詢，其中的左邊是150M，而索引鍵的基數是148M。 的右側`join`是 1.5 b，而索引鍵的基數是 ~ 100M。

具有預設`join`策略的查詢會在4分鐘後到達 Kusto 限制和超時時間。
使用隨機播放`join`策略時，查詢會在 ~ 34 秒後結束，而記憶體使用量尖峰為 1.23 GB。


下列範例顯示具有兩個叢集節點的叢集上的改善、資料表具有60M 記錄，而且索引`join`鍵的基數是2M。
執行查詢時， `hint.num_partitions`不會使用兩個分割區（作為叢集節點編號），而下列查詢將會花費 ~ 1:10 分鐘：
