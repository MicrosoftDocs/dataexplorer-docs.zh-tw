...
description: '本文描述 Azure 資料總管中 ( # A1 parse_version。'
...
ms.locfileid: "92246235"
---
# <a name="parse_version"></a>parse_version()

將版本的輸入字串表示轉換成可比較的十進位數。
...
* *`Expr`*：類型的純量運算式 `string` ，這個運算式會指定要剖析的版本。

> [!NOTE]
> * 輸入字串必須包含一到四個版本部分（以數位表示），並以點分隔 ( '. ') 。
> * 版本的每個部分最多可包含八位數，最大值為99999999。
> * 如果部分數目小於四個，所有遺漏的零件都會被視為尾端的 (`1.0`  ==  `1.0.0.0`) 。
