...
第一個匯總子句 (最左邊的) 根據這些記錄上某個運算式的唯一值，將輸入記錄會細分到資料分割中。 然後，子句會保留特定數目的記錄，將此運算式最大化或最小化到記錄上。 接著，下一個匯總子句會以嵌套的方式套用類似的函式。 下列子句會套用至上一個子句所產生的資料分割。 此程式會針對所有匯總子句繼續進行。
...
* *`Expr`*：輸入記錄的運算式，指出要為此階層層級傳回哪個值。
  一般而言，它是表格式輸入 (*T*) 的資料行參考，或某些計算 (例如 `bin()`) 資料行。
...
* *`Aggregation`*：數值運算式，表示要套用至所有共用相同值之記錄的匯總 *`Expr`* 。 此匯總的值會決定哪一個產生的記錄是「最上層」。
   ...
   * [平均 ( # B1 ](avg-aggfunction.md)、
   * [百分位數 ( # B1 ](percentiles-aggfunction.md)，和
   * [percentilew ( # B1 ](percentiles-aggfunction.md)。 也支援匯總的任何代陣列合。

* `asc`或者 `desc` (預設) 可能會出現，以控制選取實際上是來自匯總值範圍的「底部」或「頂端」。

## <a name="returns"></a>傳回

這個運算子會傳回一個資料表，其中包含每個匯總子句的兩個數據行：

* 其中一個資料行會保存子句計算的相異值 *`Expr`* (如果指定*ExprName* ，就會將其資料行名稱命名) 

* 一個資料行保存*匯總*計算的結果 (如果指定了資料行名稱，則為*AggregationName*) 
...
* 省略*N*的值
* 使用資料行名稱做為的值*`Expr`*
* 會使用 `Ignore=max(1)` 做為匯總，然後忽略資料行的 (或專案外) `Ignore` 。

記錄的數目可能會隨著匯總子句數目以指數方式成長 ( # B1 N1 + 1) \* (N2 + 1) \* ... ) 。如果未指定*N*限制，記錄成長速度會更快。 請考慮此操作員可能會耗用大量資源。

如果匯總的散發量很大，請使用*N*) 來限制要傳回 (的相異值數目，並使用 `with others=` *ConstExpr*選項來取得所有其他情況的「權數」指示。
...
為每個群組 (此層級中的每個值提供索引排序次序) 在此範例中的最後一個嵌套層級 (排序結果，方法是 EndLocation) ：
