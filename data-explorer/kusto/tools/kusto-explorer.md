...
|長條圖    | 顯示橫條圖，其中所有數值資料行都對應到不同的數列（X 軸）。 數值前面的文字資料行是 Y 軸（可在 UI 中控制）|
|堆疊區域圖      | 顯示堆疊區域圖，其中 X 軸是第一個資料行（必須是數值）。 所有數值資料行都會對應到不同的數列（Y 軸） |
|時間軸圖表   | 顯示 X 軸為第一個資料行（必須是 datetime）的時間圖表。 所有數值資料行都會對應到不同的數列（Y 軸）。|
|折線圖   | 顯示折線圖，其中 X 軸是第一個資料行（必須是數值）。 所有數值資料行都會對應到不同的數列（Y 軸）。|
...
|時間階梯 |    顯示「階梯圖表」，其中 X 軸是最後兩個數據行（必須是 datetime）。 Y 軸是其他資料行的複合。|
...
1. 固定間隔值區中的時間
2. 要在 Kusto 中產生時間序列資料的異常偵測的數值，以 [時間] 欄位摘要，並指定 [時間段] bin。
...
(*)資料檢視設定： Kusto 會持續追蹤每一組唯一資料行所使用的設定。 重新排列或移除資料行時，會儲存資料檢視，並在每次抓取具有相同資料行的資料時重複使用。 若要將視圖重設為預設值，請在 [**視圖**] 索引標籤中選取 [**重設視圖**]。 
...
[連線] 窗格會顯示所有已設定的叢集連接。 針對每個叢集，會顯示其所儲存的資料庫、資料表和屬性（資料行）。 選取 [專案] （在主面板中設定搜尋/查詢的隱含內容），或按兩下 [專案] 將名稱複製到 [搜尋/查詢] 面板。

如果實際的架構很大（例如具有數百個數據表的資料庫），您可以按下**CTRL + F**並輸入您要尋找之機構名稱的子字串（不區分大小寫）來搜尋它。
...
例如，存在於多個 AAD 租使用者中的使用者，有時需要將其身分識別的特定「投射」用於特定 AAD 租使用者。 若要這麼做，請提供連接字串，如下所示（以特定值取代大寫的單字）：

```kusto
Data Source=https://CLUSTER_NAME.kusto.windows.net;Initial Catalog=DATABASE_NAME;AAD Federated Security=True;Authority Id=AAD_TENANT_OF_CLUSTER;User=USER_DOMAIN
```

* `AAD_TENANT_OF_CLUSTER`這是主控叢集之 AAD 租使用者的功能變數名稱或 AAD 租使用者識別碼（GUID）。 這通常是擁有叢集之組織的功能變數名稱，例如 `contoso.com` 。 
...
Kusto 會嘗試解讀 [結果] 面板中每個資料列的嚴重性或詳細等級，並據以調整其色彩。 其方式是將每個資料行的相異值與一組已知模式（「警告」、「錯誤」等等）進行比對。
