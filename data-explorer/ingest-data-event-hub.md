...
description: 在本文中，您將瞭解如何將 (從事件中樞將) 資料載入至 Azure 資料總管。
author: orspod
ms.author: orspodek
...
Azure 資料總管可從事件中樞、巨量資料串流平台及事件內嵌服務進行內嵌 (載入資料)。 [事件中樞](/azure/event-hubs/event-hubs-about)可以近乎即時地每秒鐘處理數百萬個事件。 在本文中，您會建立事件中樞、從 Azure 資料總管連線到它，並查看整個系統的資料流程。
