...
| 已處理的批次 | 計數 | Avg、Max、Min | 針對內嵌完成的批次數目。 `BatchCompletionReason`：批次是否達到批次處理時間、資料大小或檔案數目限制（依 [批次處理原則](/azure/data-explorer/kusto/management/batchingpolicy)設定）。 | Database、BatchCompletionReason |
