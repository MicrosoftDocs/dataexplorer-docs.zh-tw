...
1. 在 [ **輸出設定** ] 窗格中，填寫下欄欄位：
    ...
    |**設定** | **建議的值** | **欄位描述**
    |---|---|---|
    | Load Balancer | 您的引擎或 *資料管理* Load Balancer | 選取為叢集引擎建立的負載平衡器，您可以使用外部或內部負載平衡器。  負載平衡器引擎名稱會採用下列格式： <br> *kucompute-{clustername}-* 適用于外部負載平衡器的 elb <br> *kucompute-{clustername}-* 內部負載平衡器的 ilb<br> *負載平衡器的資料管理名稱將採用下列格式： <br> kudatamgmt-{clustername}-elb <br> kudatamgmt-{clustername}-ilb*|
