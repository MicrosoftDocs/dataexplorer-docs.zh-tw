...
ms.locfileid: "88874030"
---
# <a name="create-an-azure-data-explorer-cluster-in-your-virtual-network"></a>在您的虛擬網路中建立 Azure 資料總管叢集

Azure 資料總管支援將叢集部署到虛擬網路中的子網 (VNet) 。 這項功能可讓您從 Azure 虛擬網路或內部部署私人存取叢集、存取資源（例如事件中樞和虛擬網路內的儲存體），以及限制輸入和輸出流量。
...
1. 在 [ **建立網路安全性群組** ] 視窗中，填寫下列資訊。
    ...
    **設定** | **建議的值** | **欄位描述**
    ...
    | 名稱 | AzureDataExplorerNsg | 選擇名稱，以識別資源群組中 (NSG) 的網路安全性群組。  |
    | 區域 | *美國西部* | 選取最符合您需求的區域。
    | | | |
...
1. 根據 [VNet 部署](vnet-deployment.md#dependencies-for-vnet-deployment)的相依性，針對所有輸入和輸出相依性重複上述兩個步驟。 或者，您可以使用單一規則來取代輸出規則，以允許埠443和80的 *網際網路* 。
...
1. 在 [ **建立公用 IP 位址** ] 窗格中，完成下列資訊。
    ...
    **設定** | **建議的值** | **欄位描述**
    |---|---|---|
    | IP 版本 | IPv4 | 選取 IP 版本。 我們僅支援 IPv4。|
    | SKU | 標準 | 我們需要 query (Engine) URI 端點的 **標準** 。 |
    | 名稱 | 引擎-pip | 選擇識別資源群組中公用 IP 位址的名稱。
    | 訂用帳戶 | 您的訂用帳戶 | 選取您要用於公用 IP 的 Azure 訂用帳戶。|
    | 資源群組 | 您的資源群組 | 使用現有資源群組，或建立新的資源群組。 |
...
1. 在 [ **建立虛擬網路** ] 視窗中，完成下列資訊。
    ...
    **設定** | **建議的值** | **欄位描述**
    ...
    | 訂用帳戶 | 您的訂用帳戶 | 選取您要用於叢集的 Azure 訂用帳戶。|
    | 資源群組 | 您的資源群組 | 使用現有資源群組，或建立新的資源群組。 |
    | 名稱 | AzureDataExplorerVnet | 選擇可識別資源群組中虛擬網路的名稱。
    | 區域 | *美國西部* | 選取最符合您需求的區域。
    | | | |

    > [!NOTE]
    > 針對生產工作負載，請根據[VNet 中的規劃子網大小](vnet-deployment.md#plan-subnet-size-in-your-vnet)來規劃子網大小
...
使用一組定義的計算和儲存體資源在 Azure 資源群組中建立 Azure 資料總管叢集，如 [建立](create-cluster-database-portal.md#create-a-cluster)叢集所述。

1. 在完成叢集建立之前，請在 [ **建立 Azure 資料總管** 叢集] 視窗中，選取 [ **網路** ] 索引標籤，使用在先前的索引標籤中建立的資源來提供虛擬網路詳細資料：

   ![建立叢集 vnet 表單](media/vnet-create-cluster-portal/create-cluster-form-vnet.png)

    **設定** | **建議的值** | **欄位描述**
    |---|---|---|
    | 訂用帳戶 | 您的訂用帳戶 | 選取您要用於網路資源的 Azure 訂用帳戶。|
...
> [將 Azure 資料總管部署到您的虛擬網路](vnet-deployment.md)
