...
對於想要擁有管理 (節流、重試、監視、警示及更多) 外部服務完成的組織而言，使用連接器可能是最適當的解決方案。 已排入佇列的擷取適合大量資料。 Azure 資料總管支援下列 Azure Pipelines：
...
* **[電源自動化](https://flow.microsoft.com/)**： Azure 資料總管的自動化工作流程管線。 電源自動化可用於執行查詢，並使用查詢結果做為觸發程式來執行預設動作。 請參閱 [Azure 資料總管連接器，以將 (Preview) 自動化 ](flow.md)。
...
有一些方法可透過 Kusto 查詢語言 (KQL) 命令，將資料直接內嵌至引擎。 因為這個方法會略過資料管理服務，所以只適用于探索和原型設計。 請勿在生產或高容量的案例中使用此方法。
  ...
  * **從儲存體內嵌 (提取) **：控制項命令 [。內嵌至](kusto/management/data-ingestion/ingest-from-storage.md) 會傳送至引擎，並將資料儲存在某些外部儲存體 (例如，Azure Blob 儲存體) 可供引擎存取，並由命令指向。

## <a name="comparing-ingestion-methods-and-tools"></a>比較內嵌方法和工具

| 內嵌名稱 | 資料類型 | 檔案大小上限 | 串流，批次處理，direct | 最常見的案例 | 考量 |
| --- | --- | --- | --- | --- | --- |
| [**一次按一下內嵌**](ingest-data-one-click.md) | * sv，JSON | 未壓縮的 1 GB (請參閱注意) | 批次處理至容器、本機檔案和直接內嵌中的 blob | 一次性、建立資料表架構、使用事件方格進行連續內嵌的定義、使用容器 (最多10000個 blob 的大量內嵌)  | 從容器隨機選取 10000 blob|
| [**LightIngest**](lightingest.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 透過 DM 或直接內嵌到引擎的批次處理 |  資料移轉、已調整的內嵌時間戳記的歷程記錄資料、大量內嵌 (沒有大小限制) | 區分大小寫，區分空格 |
| [**ADX Kafka**](ingest-data-kafka.md) | | | | |
| [**ADX 至 Apache Spark**](spark-connector.md) | | | | |
| [**LogStash**](ingest-data-logstash.md) | | | | |
| [**Azure Data Factory**](kusto/tools/azure-data-factory.md) | [支援的資料格式](/azure/data-factory/copy-activity-overview#supported-data-stores-and-formats) | 每個 ADF 限制 (無限制)  | 批次處理或每個 ADF 觸發程式 | 支援通常不受支援的大型檔案格式，可從永久至雲端的超過90來源複製 | 內嵌時間 |
|[**Azure 資料流程**](kusto/tools/flow.md) | | | | 在流程中內嵌命令| 必須具有高效能回應時間 |
| [**IoT 中樞**](ingest-data-iot-hub-overview.md) | [支援的資料格式](ingest-data-iot-hub-overview.md#data-format)  | N/A | 批次處理，資料流程 | IoT 訊息，IoT 事件，IoT 屬性 | |
| [**事件中樞**](ingest-data-event-hub-overview.md) | [支援的資料格式](ingest-data-event-hub-overview.md#data-format) | N/A | 批次處理，資料流程 | 訊息、事件 | |
| [**事件方格**](ingest-data-event-grid-overview.md) | [支援的資料格式](ingest-data-event-grid-overview.md#data-format) | 未壓縮 1 GB | 批次處理 | 從 Azure 儲存體、Azure 儲存體中的外部資料進行連續內嵌 | 100 KB 是最佳的檔案大小，用於 blob 重新命名和建立 blob |
| [**Net Std**](net-standard-ingest-data.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 批次處理、串流、直接 | 根據組織需求撰寫您自己的程式碼 |
| [**Python**](python-ingest-data.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 批次處理、串流、直接 | 根據組織需求撰寫您自己的程式碼 |
| [**Node.js**](node-ingest-data.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱附注 | 批次處理、串流、直接 | 根據組織需求撰寫您自己的程式碼 |
| [**Java**](kusto/api/java/kusto-java-client-library.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 批次處理、串流、直接 | 根據組織需求撰寫您自己的程式碼 |
| [**REST**](kusto/api/netfx/kusto-ingest-client-rest.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 批次處理、串流、直接| 根據組織需求撰寫您自己的程式碼 |
| [**Go**](kusto/api/golang/kusto-golang-client-library.md) | 支援的所有格式 | 未壓縮的 1 GB (請參閱注意)  | 批次處理、串流、直接 | 根據組織需求撰寫您自己的程式碼 |
...
1. **建立架構對應**
    ...
    [架構對應](kusto/management/mappings.md) 有助於將源資料欄位系結至目的地資料表資料行。 對應可讓您根據定義的屬性，將不同來源的資料匯入相同的資料表。 支援不同類型的對應，也就是資料列導向 (CSV、JSON 和 AVRO) ，以及資料行導向的 (Parquet) 。 在大部分的方法中，您也可以在 [資料表上預先建立](kusto/management/create-ingestion-mapping-command.md) 對應，並從內嵌命令參數參考。

1. **將更新原則設定** (選擇性) 

    (Parquet、JSON 和 Avro 的部分資料格式對應) 支援簡單且有用的內嵌時間轉換。 案例在內嵌時需要更複雜的處理，請使用更新原則，這可讓您使用 Kusto 查詢語言命令進行輕量處理。 更新原則會自動對原始資料表上的內嵌資料執行提取和轉換，並將產生的資料內嵌到一個或多個目的地資料表。 設定您的 [更新原則](kusto/management/update-policy.md)。
